...
- replace the mirror-managed Prettier pre-commit hook with the repository-local Prettier dependency, install locked npm dependencies during hook setup, and approve required esbuild and macOS fsevents install scripts so clean-clone setup works without npm 12 warnings
